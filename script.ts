...
document.addEventListener('DOMContentLoaded', () => {
  ...
  inits.forEach(fn => {
    try { fn(); }
    catch (err) { console.error(`[UserTechEvolution] Falha ao iniciar ${fn.name}:`, err); }
  });
});
...
function initCounters(): void {
  const counters = document.querySelectorAll<HTMLElement>('.stat__number');
...
/* ---------- Partícula usada pela rede neural de fundo e pela mini-demo ---------- */
interface Particle {
  x: number;
  y: number;
  vx: number;
...
function initNeuralNetwork(): void {
  ...
  let width: number, height: number, nodes: Particle[], nodeCount: number;
  ...
  function createNodes(): void {
    nodes = Array.from({ length: nodeCount }, (): Particle => ({
      x: Math.random() * width,
...
function initTypedTerminal(): void {
...
function initPortfolioFilters(): void {
  ...
  const cards = document.querySelectorAll<HTMLElement>('#portfolioGrid .project-card');
...
/* ---------- Project detail modal (case studies) ---------- */
...
function initProjectModal(): void {
  ...
  const openTriggers = document.querySelectorAll<HTMLElement>('.js-open-modal');
  const closeTriggers = modal.querySelectorAll<HTMLElement>('[data-close-modal]');

  const els = {
    image: document.getElementById('modalImage') as HTMLImageElement | null,
  ...
  function openModal(key: string | undefined): void {
    if (!key) return;
    const data = PROJECTS[key];
    if (!data || !els.image || !els.category || !els.title || !els.challenge || !els.solution || !els.result || !els.tags) return;
...
function initPricingToggle(): void {
  ...
  const amounts = document.querySelectorAll<HTMLElement>('.price-card__amount[data-cash]');
  const periods = document.querySelectorAll<HTMLElement>('.price-card__period');
  const labels = document.querySelectorAll<HTMLElement>('.pricing__toggle-label');
  ...
  toggle.addEventListener('click', () => {
    ...
    amounts.forEach(el => {
      el.style.opacity = '0';
      setTimeout(() => {
        el.textContent = next ? (el.dataset.installment ?? '') : (el.dataset.cash ?? '');
        el.style.opacity = '1';
      }, 150);
    });
    ...
    periods.forEach(el => {
      ...
      if (!valueBox || !valueBox.querySelector('.price-card__amount[data-cash]')) return;
      el.style.opacity = '0';
      setTimeout(() => {
...
function initPlanSelection(): void {
  const buttons = document.querySelectorAll<HTMLElement>('.js-select-plan');
...
function initFlipCardsTouch(): void {
  if (!window.matchMedia('(hover: none)').matches) return;
  document.querySelectorAll<HTMLElement>('.ai-flip').forEach(card => {
    card.addEventListener('click', () => {
      document.querySelectorAll<HTMLElement>('.ai-flip.is-flipped').forEach(other => {
...
function initLogoRipple(): void {
  const logoLink = document.getElementById('logoLink');
  const svg = logoLink?.querySelector('.logo-svg');
  if (!logoLink || !svg) return;
...
function initScrollSpy(): void {
  ...
  links.forEach(link => {
    ...
    if (href && href.startsWith('#')) linkBySectionId.set(href.slice(1), link);
  });

  const observer = new IntersectionObserver((entries: IntersectionObserverEntry[]) => {
    entries.forEach(entry => {
      const link = linkBySectionId.get((entry.target as HTMLElement).id);
...
function initNavIndicator(): void {
  ...
  const links = list.querySelectorAll<HTMLAnchorElement>('.nav__link');
  ...
  nav.addEventListener('mouseleave', () => {
    const active = list.querySelector<HTMLAnchorElement>('.nav__link.is-active');
...
function initDemoPreview(): void {
  ...
  const canvas = document.getElementById('demoCanvas') as HTMLCanvasElement | null;
  const statNumbers = document.querySelectorAll<HTMLElement>('.demo-stat__number');
  ...
  if (statNumbers.length && 'IntersectionObserver' in window) {
    ...
    const obs = new IntersectionObserver((entries: IntersectionObserverEntry[]) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) { animate(entry.target as HTMLElement); obs.unobserve(entry.target); }
      });
    }, { threshold: 0.6 });
  ...
  if (canvas) {
    const ctx = canvas.getContext('2d');
    const screen = canvas.closest<HTMLElement>('.browser-mock__screen');
    if (ctx && screen) {
      const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      let width: number, height: number, nodes: Particle[];
      const COLORS = ['#3E7BFA', '#8B5CF6', '#00E5FF'];
      ...
      const resize = (): void => {
        ...
        nodes = Array.from({ length: count }, (): Particle => ({
          x: Math.random() * width,
          y: Math.random() * height,
      ...
      const loop = (): void => { draw(); if (!prefersReducedMotion) requestAnimationFrame(loop); };

      resize();
      window.addEventListener('resize', debounce(resize, 250));
      if (prefersReducedMotion) draw(); else requestAnimationFrame(loop);
    }
  }
}
...
function initConfetti(): void {
  ...
  const observer = new MutationObserver(() => {
    if (status.style.color === 'rgb(0, 229, 255)' && status.textContent?.trim()) {
      burst();
    }
  });
